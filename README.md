...
| **12** | **完了確認** | 「3. 学習の実行」セルの出力に<br>**`# 学習完了 — Hugging Face の「モデル ID」`** が出れば成功 |

**学習データ**はノートブック **「2.5 学習データの指定（毎回）」**で **`.jsonl` をアップロード（またはローカルでファイル選択）**して指定する（Alpaca 形式）。**セクション 3 の前に必ず 2.5 を実行**する。

**推論セクション（4）** を含めて「すべて実行」すると、最後で **`あなた>` の入力待ち**になります。推論だけ試すときは学習後に **セクション 4 だけ**実行してもよいです。
